        ...
        {clues.map((clue, idx) => {
          ...
          return (
            <div
              ...
              className={`relative ${isUnlocked ? "animate-fileFlip" : ""}`}
            >
              {isUnlocked || isAnswered ? (
                <div
                  className={`relative rounded-sm border border-ink/10 bg-parchment/50 overflow-hidden ${
                ...
                >
                  <div className="flex-shrink-0 w-7 h-7 rounded-full bg-ink/5 flex items-center justify-center group-hover:bg-gold/15 transition-colors">
                    <Lock size={12} className="text-ink/40 group-hover:text-gold" />
                  </div>
                  <div className="flex-1 min-w-0">
                    <div className="text-[10px] uppercase tracking-widest text-ink/40 font-serif mb-0.5">
                      {meta.label} · 待解锁
                    </div>
                    <div className="text-sm text-ink/60 group-hover:text-ink font-serif">
                      查看「{clue.label}」相关线索
                    </div>
                  </div>
                  <ChevronRight
